...
The following five packages are licensed under the **Apache License,
...
## Apache-2.0 components — bundled LLM weights (shipped)

The shipped model files are downloaded into the user's `models/` directory
on first run and are subject to their upstream licenses. The two LLM
families currently shipped are released under Apache 2.0:

### Qwen3-4B and Qwen3-8B (GGUF, Q4_K_M)

- Copyright (c) Alibaba Cloud (Qwen Team)
- Source: https://huggingface.co/Qwen
- Use in LokLM: local LLM ("lite" and "medium" tiers).

---

## MIT components — bundled embedding/reranker weights (shipped, courtesy)

MIT does not require a separate NOTICE entry, but BAAI's BGE models are
...
- Use in LokLM: workspace embedding + cross-encoder reranking.
