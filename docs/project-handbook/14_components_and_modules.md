...
| **Verarbeitung** | `parseFile` (PDF via `pdf-parse`/`pdfjs`, Markdown, Text, OCR via Tesseract für Bilder/gescannte PDFs) → `chunkPages`/`chunkMarkdown` (Default 2000 Zeichen, 200 Overlap, Separator-Kaskade) → Sprach-Tagging (`eld`) → `persistChunks` (Batch 200) → Embedding (Batch 32). Hash-bewusster Refresh-Kurzschluss (`contentHash`/`sourceMtime`) |
