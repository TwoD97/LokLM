...
| 13   | Technische Architektur             | zu pruefen   | ja (Quellcode, ADRs)                                       | IPC-Handler-Gesamtzahl geklärt → **105** (Stand `main` v0.4.7 `d35e219`, = Kap. 13; auf v0.4.6 `af59c25` waren es 103); ADR-0004-Implementierungsstand geklärt → NICHT implementiert (kein `placement/`-Code), reiner Design-Vorschlag (PROPOSED) | niedrig |
...
| 19   | Testing & QA                       | zu pruefen   | ja (Test-READMEs, Abschluss-Dokus)                         | M-Szenarien nicht protokolliert; CI-Test-Job seit #19-Merge auf main; Coverage-Gate offen                                                                                                                                                         | hoch    |
...
| 24   | Status nach 9 Wochen               | zu pruefen   | ja (Statusbericht 2026-06-14)                              | AP-E.2 Phase 2; Dependabot (vite/vitest geschlossen, astro #15 offen); gitignored-Quellen                                                                                                                                                         | hoch    |
...
3. **Versions-/Benennungs-Klärungen** → v0.4.2-Tag (Kap. 02, 09), Vorname Projekt-Owner
   (Kap. 00, 07).
4. **CI-/Test-Verankerung** → vitest-CI-Job (`integration + tx`) seit Merge von PR #19 (2026-06-16) auf `main` aktiv (Kap. 19, 22, 24); offen bleibt nur ein dauerhaftes Coverage-Threshold-Gate.
