...
Das Handbuch hat den Stand **2026-06-16** (Handbuch-Version 0.1); die Status- und Projektangaben bilden ca. **9 Wochen Projektarbeit** (Stand 2026-06-14) ab, die technischen Kapitel sind mit dem aktuellen `main`-Stand (**v0.4.7**, Commit `d35e219`) abgeglichen — gegenüber v0.4.6 nachgezogen wurden u. a. die LLM-Geräteauswahl (`eba08e3`), die Vault-Crash-Resilienz (`f4009b1`), die **Entfernung der In-App-Erststart-Modell-Download-Ansicht** (Modelle liefert nun das Installer-Tier-Bundle; `aeccfd9`) sowie ein GPU-Packaging-Fix (übersprungener `node-llama-cpp`-Selbsttest, damit die GPU im Package lädt; `280110b`/`432ae29`). Es bezieht sich auf den getaggten Release-Stand bis einschließlich **v0.4.1** (v0.4.2–v0.4.7 sind Release-Commits ohne Tag) und auf den **Branch-Stand der Test-/Eval-Säule**: Die Deliverables **AP-T.1, AP-T.2 und AP-E.1 sind inzwischen nach `main` gemergt** (PRs #24/#19/#25, 2026-06-16); **AP-E.1b** (PR #28) und **AP-E.2** (`dom/ap-e2-matrix-eval`) liegen weiterhin auf eigenen Feature-Branches. Die für AP-E.2 noch branch-abhängigen Kapitel (insbesondere 11, 15, 17, 18, 23) dokumentieren diesen Branch-Stand und sind entsprechend markiert. Der Pflichtenheft-Stand ist Version 1.1.2 (Nachtrag Sprint-4/5).
...
| ----------------- | ---------------------------------------------------------- | ----------- |
| 0.1               | v0.4.7 (`d35e219`) zzgl. Branch-Stand der Test-/Eval-Säule | 2026-06-16  |
