...
> ⚠ zu verifizieren — Der Kommentar in `matrixConfigs()` (`configs.ts`) spricht von „8 embedder × (1 skip + 2 reranker) × 3 chunker = 72 configs". Die tatsächliche Chunker-Achse `MATRIX_CHUNKER_SPECS` enthält jedoch **nur eine** Größe (`fixed-512-64`), weil `sweep.ts` **nicht** pro Config re-chunkt (es nutzt die vor-gechunkten `dataset.chunks`); eine Mehr-Größen-Achse hier wäre wirkungslos (3× identische Ergebnisse). Der Chunk-Größen-Vergleich läuft korrekt als **separate Läufe** über die chunker-unabhängige Span-Metrik (siehe 17.4 und Kapitel 15.7). Der „72"-Kommentar im Quelltext ist insofern veraltet/irreführend und sollte auf 24 korrigiert werden.
