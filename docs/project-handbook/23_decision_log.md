...
Quellen: `docs/adr/0001`–`0004`, `docs/adr/README.md`, `docs/work/projektstatusbericht-2026-06-07.md`, `docs/work/projektstatusbericht-2026-06-14.md`, `tests/evals/pipeline/configs.ts` (`matrixConfigs()`) sowie — noch nicht nach `main` gemergt, Branch `dom/ap-e2-matrix-eval` — `tests/evals/answer/matrix-manifest.ts`, `tests/evals/license/validate-model-licenses.ts`, `tests/evals/model-license-registry.json`, Memory `ap-ticket-abschluss-doku.md` / `no-internal-docs-in-repo.md`.

> WARN zu verifizieren — AP-E.2-Eval-Artefakte (Lizenz-Validator, Registry, Matrix-Manifest) liegen auf Branch `dom/ap-e2-matrix-eval` und sind in `main` nicht vorhanden.

> WARN zu verifizieren — Manifest-Builder (`matrix-manifest.ts`) erst auf `dom/ap-e2-matrix-eval`; in `main` realisiert `matrixConfigs()` in `tests/evals/pipeline/configs.ts` die Matrix.
...
| **Konsequenz** | DEK-Rotation nicht möglich ohne Re-Encrypt (akzeptiert); Korruption des Primärfiles wird seit v0.4.2 durch eine automatische Byte-Kopie `loklm.vault.bak` (Load-/Reset-Fallback, deckt AV-Quarantäne, manuelles Löschen, Bit-Rot) abgefedert (`src/main/services/auth/AuthService.ts`, `writeVaultNow`/`vaultBackupPath`/`readVault`); Total-Verlust erst, wenn beide Dateien fehlen. Eine vollständige Backup-/Export-UI bleibt separate Spec. ADR-0002 noch auf altem Stand („Korruption ist binär"). Magic-Byte-Versionierung manuell |
| **Status** | accepted |
...
| AP-E.2 | **Multi-Pod-Sharding der Matrix** | 315 Zellen × 163 Fragen = hoher GPU-Bedarf | Seriell auf einem Pod | Round-robin-Shards (`parseShard`/`selectShard`), Union = volle Menge, disjunkt | Sweep auf mehrere RunPod-GPUs verteilbar | umgesetzt | `matrix-manifest.ts` |
...
> WARN zu verifizieren — die exakte **315**-Zellen-Größe folgt aus den committeten OSI-Packs (7 × 3 × 1 × 15; `embedder-pack.json` = `…-osi-7`, `e5-base` entfernt); die ursprüngliche Design-Größe lag bei ~399 Zellen. Der final gefahrene Abgabe-Scope ist eine offene Team-Entscheidung (siehe Risiko R6/R8).
