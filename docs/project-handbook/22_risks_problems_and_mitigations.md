...
|---|---|---|---|---|---|---|---|
| R1 | **Partnerseitiger Ausfall (KW 23)** | Projekt-Owner Denys Tudosa (Chunking/Auth/RAG/Installer) zeitweise nicht eingebunden | Integration/Auslieferung stockte, Gesamtstatus *kritisch* | Wiedereinstieg; zwei Releases (v0.4.0/v0.4.1) + AP-6/AP-9-Merges nachgeholt; Status auf *planmäßig* zurückgestuft | **gelöst** | gering — Bus-Faktor bleibt bei 2 Personen | `projektstatusbericht-2026-06-14.md` |
...
| R15 | **Determinismus-Flakes in Tests** | `searchChunks` ohne Tie-Break; fixes `setTimeout(2000)` in `document-import.test.ts` | Latente Test-Flakes / Cross-Session-Nichtdeterminismus | An Denys (Logik-Domäne) gemeldet: zweiter Sortierschlüssel + Polling-`waitFor` empfohlen | **offen (gemeldet)** | gering | `ap-t2-abschluss-doku.md` §3 |
